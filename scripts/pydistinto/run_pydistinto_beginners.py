...
#datafolder = join(outputdir, "results_not_normalized", "")
resultsfolder = join(outputdir, "results_dispersion_1s_1000f_1000s_1f", "")
plotfolder = join(outputdir, "plots", "")
if use_randomized_texts == "yes":
    taggedfolder = join(outputdir, "randomized", "")
else:
    taggedfolder = join(outputdir, "synthetic_same_length", "")   #synthetic text corpus
...
for i in range(0,100):
    absolutefreqs, relativefreqs, binaryfreqs, absolutefreqs_sum, tf_frame, liste1, liste2 = prepare.main(taggedfolder, segmentfolder, dtmfolder, segmentlength, max_num_segments, stoplistfile, featuretype)
...
#contrast = ["random", "two", "one"] # for splitting groups randomly
    ...
    calculate_simple.main(dtmfolder, metadatafile, separator, contrast, logaddition, resultsfolder, segmentlength, featuretype, absolutefreqs, relativefreqs, binaryfreqs, tf_frame, liste1, liste2, i)
